...
mod_GSAcc_server <- function(input, output, session, parent_session){
    ...
    observe({
      ...
      if (advanced_options$pred_model == "rrBLUP"){
        ...
        results <- genomic_prediction(geno_adj, pheno,
                                      traits = traits,
    ...
    plots <- reactive({
      ...
      pred_outputs$box_plot <- ggplot(df_long, aes(x = "rrBLUP", y = Correlation, fill = "red"), fill = "red") +
      ...
      pred_outputs$violin_plot <- ggplot(df_long, aes(x = "rrBLUP", y = Correlation, fill = "red")) +
