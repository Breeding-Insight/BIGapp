  ...
  tagList(
    ...
    fluidRow(
      ...
      column(width = 3,
             box(
               ...
               conditionalPanel(condition=("input.passportImportType==2"),
                                ...
                                actionButton(ns("submit_token"), "Submit Token"),
                                ns=ns
               ),
      ...
      column(width = 8,
             ...
             box(title = "Passport Data BrAPI", width = 12, solidHeader = TRUE, collapsible = TRUE, status = "info", collapsed = FALSE,
                 DTOutput(ns('passport_table_brapi')),
                 style = "overflow-y: auto; height: 480px"
             ),
             box(
...
mod_PCA_server <- function(input, output, session, parent_session){
  ...
  passport_table_brapi <- reactive({
    
    validate(
      need(!(input$brapi_token=='' && !is.null(brapi_data$dftest)), "Enter BrAPI token to access results in this section."),
    )
  ...
  output$passport_table_brapi <- renderDT({
    passport_table_brapi()},
  ...
  observeEvent(input$submit_token, {
    validate(
      need(!(input$brapi_token==''), "Enter BrAPI token to access results in this section."),
    )
    
    connection <- brapi_connect(
      db = "localhost",
      port = 8081,
      secure = FALSE,
      apipath = "v1/",
      token = input$brapi_token,
    )
    
    print(brapi_checkCon(connection, verbose = FALSE))
    
    #TODO still need to be able to retrieve program id associated with token
    #may need to fork brapir as brapi_get_programs is not pinging the endpoint we need
    #browser()
    #retrieve new api path from program id
    #df0 <- brapi_get_programs(con = connection)
    #print(df0)
    
    newapipath <- paste('v1/programs', '3c150628-1788-4241-9785-9a53e917a5ad', sep='/')
    
    connection['apipath'] <- newapipath
    
    #TODO need better checks, this just tests if initial connection params work, need something to verify token
    #as is, if GET fails it just crashes with an unhelpful error cause there's no catch alongside the try, so it gives an unhelpful object 'out' not found error
    #because out object created in try statement and then accessed afterward
    #validate(
    # need(brapirv1::brapi_checkCon(connection, verbose = FALSE), "BrAPI connection failed. Check token."),
    #)
    
    #retrieve samples
    #browser()
    df1 <- brapi_get_samples(con = connection)
    brapi_data$dftest <- df1
    #TODO need to filter samples, as is pinging the endpoint is bringing in samples outside the program
    #extra samples show up when using bruno to ping endpoint
    #determine whether to modify api call or filter downstream
    #"referenceSource": "breedinginsight.org/samples" filter
    
    #TODO filter table columns
    #TODO determine if data should be wiped/replaced when swapping between brapi and file upload or just when submitting
  }
  )
